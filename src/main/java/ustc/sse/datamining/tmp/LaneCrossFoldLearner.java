 ...
 */
public class LaneCrossFoldLearner extends AbstractVectorClassifier implements OnlineLearner, Writable {
  private int record;
  ...
  public LaneCrossFoldLearner() {
  }

  public LaneCrossFoldLearner(int folds, int numCategories, int numFeatures, PriorFunction prior) {
  ...
  public LaneCrossFoldLearner lambda(double v) {
  ...
  public LaneCrossFoldLearner learningRate(double x) {
    for (OnlineLogisticRegression model : models) {
  ...
  public LaneCrossFoldLearner stepOffset(int x) {
    for (OnlineLogisticRegression model : models) {
      model.stepOffset(x);
    }
    return this;
  ...
  public LaneCrossFoldLearner decayExponent(double x) {
    for (OnlineLogisticRegression model : models) {
  ...
  public LaneCrossFoldLearner alpha(double alpha) {
    for (OnlineLogisticRegression model : models) {
      model.alpha(alpha);
    }
    return this;
  ...
  public void train(long trackingKey, String groupKey, int actual, Vector instance) {
    ...
    for (OnlineLogisticRegression model : models) {
      if (k == mod(trackingKey, models.size())) {
        ...
        double score = Math.max(v.get(v.size()-1), MIN_SCORE);
//        if(score == 1){
...
//        }
  ...
  public LaneCrossFoldLearner copy() {
    LaneCrossFoldLearner r = new LaneCrossFoldLearner(models.size(), numCategories(), numFeatures, prior);
    r.models.clear();
    for (OnlineLogisticRegression model : models) {
      model.close();
      OnlineLogisticRegression newModel =
          new OnlineLogisticRegression(model.numCategories(), model.numFeatures(), model.getPrior());
